...
export const ANALYSIS_PROMPT =
  '이 프로젝트의 README, 최근 git 커밋 로그, 소스 파일 구조를 읽고, 이 프로젝트가 뭘 하는 프로젝트인지와 지금 어디까지 됐고 어디서 멈췄는지를 요약해줘. 확인했다는 말이나 서론 없이 바로 시작할 것. 2~3문장, 전체 150자 안팎으로 쓸 것. 어떤 기능이 있고 최근에 뭘 하다가 멈췄는지는 구체적으로 말하되, 커밋 해시나 파일 경로, 스크립트 이름 같은 지엽적인 기술 디테일만 빼고 말할 것. 마크다운 형식이나 글머리 기호는 쓰지 말고, 완성도를 숫자나 퍼센트로 매기지 말고, 이 프로젝트를 계속하는 게 좋을지 말지는 추천하지 마.';
