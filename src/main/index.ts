...
// last-run.json with no OS-level locking between them — possible in
// practice since the login item can auto-launch one while the user
// double-clicks the .app themselves.
...
ipcMain.handle('get-running-servers', () => getRunningServers(registry.getAll(), app.getAppPath()));

ipcMain.handle('kill-server', (_event, pid: number) => killServer(pid));
