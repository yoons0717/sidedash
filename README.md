...
macOS 메뉴바에서 여러 사이드 프로젝트를 관리하고 실행할 수 있는 개인용 Electron 대시보드입니다.

기존 프로젝트의 실행 스크립트를 그대로 활용하면서, 프로젝트 상태 확인과 실행을 메뉴바에서 처리할 수 있도록 만들었습니다.
...
여러 사이드 프로젝트를 동시에 진행하다 보니, 매번 터미널을 열어 프로젝트를 찾고 실행 명령어를 입력하거나 브랜치 상태를 확인하는 과정이 반복되었습니다.

이런 반복 작업을 줄이기 위해 메뉴바에서 프로젝트 상태를 한눈에 확인하고, 기존 스크립트를 그대로 실행할 수 있는 가벼운 런처를 만들었습니다.

## 주요 기능

- 메뉴바 트레이 아이콘으로 상주 (`menubar` 사용)
- 트레이 클릭 시 프로젝트 목록을 표시하고, 열 때마다 Git 상태를 새로고침
  - 브랜치
  - 마지막 커밋
  - 마지막 실행 시각
  - GitHub / VS Code / Finder / cmux 바로 열기
  - 최근순 / 이름순 정렬 (재시작 시 최근순으로 초기화)
- 미커밋 변경 사항이 있으면 카드에 표시하고, 클릭 시 변경 파일 목록 펼침
- ＋ / ✕ 버튼으로 프로젝트 등록 및 제거
- 프로젝트 루트 파일을 검사해 실행 가능한 액션을 자동 감지
  - `run.sh`
    - "파이프라인 실행" 버튼 표시
    - 실행 대상 프로젝트 선택 패널 제공
    - 선택한 프로젝트 경로를 `./run.sh` 인자로 전달
    - 실행될 명령을 실시간 미리보기로 표시
  - `package.json`의 `scripts.pdf`
    - "PDF 생성" 버튼 표시
    - `npm run pdf` 실행
- 액션 실행 시 로그 창에서 stdout / stderr를 실시간 스트리밍
- 완료 후 성공 / 실패 상태 표시
- 성공 시 결과 폴더(`notes/`, `pdf-output/`)를 바로 여는 버튼 제공
- 실행 중인 프로젝트는 버튼을 비활성화해 중복 실행 방지
- 완료 시 macOS 알림 표시, 클릭하면 로그 창으로 포커스 이동
- 실행 중 앱 종료 시 확인 다이얼로그를 표시하고, 확인하면 프로세스 트리를 정리한 뒤 종료
