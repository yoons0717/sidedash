...
Dock에 고정해 사용하세요. 패키징된 앱은 실행될 때마다 로그인 시 자동 실행되도록 등록되므로,
Mac을 재부팅해도 다시 켤 필요가 없습니다 (등록된 경로는 `시스템 설정 > 일반 > 로그인 항목`에서
확인/해제 가능하며, 앱을 다른 위치로 옮기면 그 위치로 다시 등록됩니다). 코드사이닝/노터라이제이션이
안 되어 있어 첫 실행 시 macOS가
